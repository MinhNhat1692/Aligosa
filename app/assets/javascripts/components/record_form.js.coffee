@FilterForm = React.createClass
	...
	handleSubmit: (e) ->
		...
		if @props.datatype == "medicine_supplier"
		...
		else if @props.datatype == "customer_search"
			switch Number($('#filter_type_select').val())
				when 1
					formData.append 'customer_name', $('#filter_text').val().toLowerCase()
				when 2
			...
			switch Number($('#filter_type_select').val())
				...
				when 6
					formData.append 'email', $('#filter_text').val().toLowerCase()
		else if @props.datatype == "project"
			switch Number($('#filter_type_select').val())
				when 1
					formData.append 'project_name', $('#filter_text').val().toLowerCase()
				when 2
					formData.append 'contractor', $('#filter_text').val().toLowerCase()
				when 3
					formData.append 'basic_info', $('#filter_text').val().toLowerCase()
				when 4
					formData.append 'address', $('#filter_text').val().toLowerCase()
	...
	triggerAutoComplete: (record) ->
		if @props.datatype == "medicine_supplier"
			...
			switch Number($('#filter_type_select').val())
				when 1
					$('#filter_text').val(record.customer_name)
				when 2
					$('#filter_text').val(record.address)
				when 3
					$('#filter_text').val(record.noid)
			...
			switch Number($('#filter_type_select').val())
				when 1
					$('#filter_text').val(record.project_name)
				when 2
					$('#filter_text').val(record.contractor)
				when 3
					$('#filter_text').val(record.basic_info)
				when 4
					$('#filter_text').val(record.address)
		else if @props.datatype == "position"
			switch Number($('#filter_type_select').val())
		...
		React.DOM.form className: 'form-horizontal row', autoComplete: 'off', onSubmit: @handleSubmit,
			React.DOM.div className: 'form-group col-lg-6 col-sm-12',
				React.DOM.div className: 'col-sm-4', style: {'marginBottom': '15px'},
					React.DOM.select id: 'filter_type_select', className: 'form-control', name: 'filterType', onChange: @triggerChangeType,
						React.DOM.option value: '', 'Chọn tiêu chuẩn lọc'
						React.DOM.option value: 1, 'Tên khách hàng'
						...
						React.DOM.option value: 6, 'Email'
				React.DOM.div className: 'col-sm-8',
					if @state.selectList == null
					...
					React.DOM.div className: "auto-complete",
						if @props.autoComplete != null
							for record in @props.autoComplete
								switch Number($('#filter_type_select').val())
									when 1
										React.createElement AutoComplete, key: record.id, record: record, trigger: @triggerAutoComplete, text: record.customer_name
									when 2
										React.createElement AutoComplete, key: record.id, text: record.address, record: record, trigger: @triggerAutoComplete
									when 3
									...
									when 6
										React.createElement AutoComplete, key: record.id, record: record, trigger: @triggerAutoComplete, text: record.email
			React.DOM.div className: 'col-lg-6 text-center',
				React.DOM.div className: 'btn-group col-sm-9 text-center',
					React.DOM.button type: 'submit', className: 'btn btn-group-left',
						React.DOM.i className: 'zmdi zmdi-search'
		...
		React.DOM.form className: 'form-horizontal row', autoComplete: 'off', onSubmit: @handleSubmit,
			React.DOM.div className: 'form-group col-lg-6 col-sm-12',
				React.DOM.div className: 'col-sm-4', style: {'marginBottom': '15px'},
					React.DOM.select id: 'filter_type_select', className: 'form-control', name: 'filterType', onChange: @triggerChangeType,
						React.DOM.option value: '', 'Chọn tiêu chuẩn lọc'
						React.DOM.option value: 1, 'Tên dự án'
						React.DOM.option value: 2, 'Tên chủ đầu tư'
						React.DOM.option value: 3, 'Thông tin cơ bản'
						React.DOM.option value: 4, 'Địa chỉ'
				React.DOM.div className: 'col-sm-8',
					if @state.selectList == null
						React.DOM.input id: 'filter_text', type: 'text', className: 'form-control', defaultValue: '', onChange: @triggerAutoCompleteInput, placeholder: 'Type here ...', name: 'filterText'
					...
					React.DOM.div className: "auto-complete",
						if @props.autoComplete != null
							for record in @props.autoComplete
								switch Number($('#filter_type_select').val())
									when 1
										React.createElement AutoComplete, key: record.id, record: record, trigger: @triggerAutoComplete, text: record.project_name
									when 2
										React.createElement AutoComplete, key: record.id, record: record, trigger: @triggerAutoComplete, text: record.contractor
									when 3
										React.createElement AutoComplete, key: record.id, record: record, trigger: @triggerAutoComplete, text: record.basic_info
									when 4
										React.createElement AutoComplete, key: record.id, record: record, trigger: @triggerAutoComplete, text: record.address
			React.DOM.div className: 'col-lg-6 text-center',
				React.DOM.div className: 'btn-group col-sm-9 text-center',
					React.DOM.button type: 'submit', className: 'btn btn-group-left',
						React.DOM.i className: 'zmdi zmdi-search'
